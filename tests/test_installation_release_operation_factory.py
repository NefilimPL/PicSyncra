...
def test_factory_accepts_only_a_signed_choice_from_the_selected_channel(tmp_path: Path, monkeypatch) -> None:
    ...
    captured = {}
    monkeypatch.setattr(module, "download_release", lambda choice, record, staging: captured.update(choice=choice, record=record, staging=staging) or {"web": Path("web.zip"), "migrator": Path("migrator.zip")})
